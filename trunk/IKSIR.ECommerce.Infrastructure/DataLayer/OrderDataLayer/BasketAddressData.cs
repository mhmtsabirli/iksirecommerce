...
{
    public class BasketAddressData
    {
        public static Address Get(int id)
        {
            ...
            SqlDataReader dr = SQLDataBlock.ExecuteReader(StaticData.Idevit.ConnectionString, CommandType.StoredProcedure, "GetBasketAddress", parameters);
            while (dr.Read())
            {
        ...
        public static int Insert(Address itemAddress, int basketId = 0, int basketItemId = 0)
        {
            ...
            returnValue = Convert.ToInt32(SQLDataBlock.ExecuteScalar(StaticData.Idevit.ConnectionString, CommandType.StoredProcedure, "SaveBasketAddress", parameters));
